...
class BTPeer:
    ...
    #--------------------------------------------------------------------------
        ...
        # if (host, int(port)) in self.peers.values():
        #     peerid = self.peers.keys()[self.peers.values().index((host, int(port)))]
        # else:
        #     peerid = None
        ...
        try:
            ...
            msgtype, msgdata, id = peerconn.recvdata()
            peerconn.id = id
    ...
    #--------------------------------------------------------------------------
        self.removepeer( self, loc ) # было до этого без self.
    ...
    #--------------------------------------------------------------------------
        ...
        try:
            ...
            peerconn.senddata( msgtype, msgdata, self.myid)
...
class BTPeerConnection:
    ...
    # any exceptions thrown upwards
        ...
        self.sd = self.s.makefile( 'rwb')


    #--------------------------------------------------------------------------
    def __makemsg( self, msgtype, msgdata, id ):
    #--------------------------------------------------------------------------
        msglen = len(msgdata)
        idlen = len(id)
        msg = struct.pack( "!4sL%dsL%ds" % (msglen, idlen), msgtype.encode('utf-8'), msglen, msgdata.encode('utf-8'), idlen, id.encode('utf-8') )
        return msg
    ...
    #--------------------------------------------------------------------------
    def senddata( self, msgtype, msgdata, id=None):
    #--------------------------------------------------------------------------
        """
        ...
        try:
            msg = self.__makemsg( msgtype, msgdata, id )
            self.sd.write( msg )
            self.sd.flush()
        except KeyboardInterrupt:
            raise
    ...
    #--------------------------------------------------------------------------
        ...
        lenstr = self.sd.read( 4 )
        idlen = int(struct.unpack( "!L", lenstr )[0])
        id = ""

        while len(id) != idlen:
            data = self.sd.read( min(2048, idlen - len(id)) )
            data = data.decode("utf-8")
            if not len(data):
                break
            id += data
